...
setwd(
  "G:/Documents/One Drive/OneDrive - UCLA IT Services/UCLA Coursework/Y1 Q3/EHS C200C/Project/Source Files"
)
...
$renter_per = census_data$renterHH / census_data$totalHH #Renters
```
...
particulatematter.data$GEOID_Numeric <-
...
small.data <-
  ...
  mutate(pop_dens_sqm = population_race/land_area)
...
## K info()Means Clustering

```{r}
small.data.k <- small.data %>% select(
  GEOID,
  population_race,
  pop_dens_sqm,
  land_area,
...
rownames(small.data.k) <- small.data.k$GEOID
...
small.data.k <- select(small.data.k,-GEOID)
...
km <- kmeans(small.data.k.scaled, centers = 4, nstart = 50)
